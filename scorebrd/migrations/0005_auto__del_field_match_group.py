...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.delete_column('scorebrd_match', 'group_id')

        # Adding M2M table for field matches on 'Group'
        db.create_table('scorebrd_group_matches', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('group', models.ForeignKey(orm['scorebrd.group'], null=False)),
            ('match', models.ForeignKey(orm['scorebrd.match'], null=False))
        ))
        db.create_unique('scorebrd_group_matches', ['group_id', 'match_id'])


    def backwards(self, orm):

        # User chose to not deal with backwards NULL issues for 'Match.group'
        raise RuntimeError("Cannot reverse this migration. 'Match.group' and its values cannot be restored.")
        # Removing M2M table for field matches on 'Group'
        db.delete_table('scorebrd_group_matches')
